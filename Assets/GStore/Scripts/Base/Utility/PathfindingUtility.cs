...
public class PathfindingUtility
{
    ...
    public static VInt3 FindValidTarget(ActorRoot actor, VInt3 start, VInt3 end, int radius, out bool bResult)
    {
        ...
        VInt3 num3 = end - start;
        VInt3 num4 = start + num3.NormalizeTo(radius);
    ...
    public static bool ValidateTarget(VInt3 loc, VInt3 target, out VInt3 newTarget, out int nodeIndex)
    {
        ...
        {
            int edge = -1;
            TriangleMeshNode node = astarData.IntersectByRasterizer(target, loc, out edge);
            if (node == null)
            {
                return false;
            }
            ...
            VInt3 result = Polygon.IntersectionPoint(ref target, ref loc, ref numArray[edge], ref numArray[(edge + 1) % 3], out intersects);
            if (!intersects)
            {
                return false;
            }
            if (!MakePointInTriangle(ref result, node, -4, 4, -4, 4, VInt3.zero))
            {
                return false;
            }
            newTarget = result;
