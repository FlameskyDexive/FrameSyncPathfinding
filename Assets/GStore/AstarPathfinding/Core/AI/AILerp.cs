    ...
    public class AILerp : VersionedMonoBehaviour, IAstarAI
    {
        ...
        Vector3 IAstarAI.steeringTarget
        {
            get
            {
                ...
                return interpolator.valid ? (Vector3)interpolator.position + (Vector3)interpolator.tangent : simulatedPosition;
            }
        }
        #endregion
        ...
        protected virtual void ConfigurePathSwitchInterpolation()
        {
            ...
            if (interpolator.valid && !reachedEndOfPreviousPath)
            {
                ...
                previousMovementOrigin = (Vector3)interpolator.position;
                //Good Game
                //previousMovementDirection = interpolator.tangent.normalized * interpolator.remainingDistance;
                previousMovementDirection = ((Vector3)interpolator.tangent).normalized * interpolator.remainingDistance;
        ...
        protected virtual void ConfigureNewPath()
        {
            ...
            if (interpolatePathSwitches && switchPathInterpolationSpeed > 0.01f && hadValidPath)
            {
                ...
                var correctionFactor = Mathf.Max(-Vector3.Dot(((Vector3)prevTangent).normalized, ((Vector3)interpolator.tangent).normalized), 0);
        ...
        protected virtual Vector3 CalculateNextPosition(out Vector3 direction, float deltaTime)
        {
            ...
            direction = (Vector3)interpolator.tangent;
            ...
            if (interpolatePathSwitches && alpha < 1f)
            {
                ...
                return Vector3.Lerp(positionAlongPreviousPath, (Vector3)interpolator.position, alpha);
            }
            else
            {
                //Good Game
                //return interpolator.position;
                return (Vector3)interpolator.position;
            }
        }
