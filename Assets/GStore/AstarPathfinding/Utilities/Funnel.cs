			...
			for (int i = 0; i < nodes.Count; i++) {
				if (nodes[i] is TriangleMeshNode || nodes[i] is GridNodeBase) {
					...
					} else
					{
                        ...
                        part.startPoint = nodes[part.startIndex-1].position;
					}
					...
					} else
					{
                        ...
                        part.endPoint = nodes[part.endIndex+1].position;
					}
                    ...
                    part.startPoint = nodes[part.startIndex].position;
					part.endPoint = nodes[part.endIndex].position;
					result.Add(part);
				} else {
		...
		public static FunnelPortals ConstructFunnelPortals (List<GraphNode> nodes, PathPart part) {
			...
			for (int i = part.startIndex; i < part.endIndex; i++) {
				...
				if (!portalWasAdded) {
                    ...
                    left.Add(nodes[i].position);
					right.Add(nodes[i].position);

					left.Add(nodes[i+1].position);
					right.Add(nodes[i+1].position);
				}
			}
