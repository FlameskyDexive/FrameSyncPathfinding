	...
	public abstract class NavmeshBase : NavGraph, INavmesh, INavmeshHolder, ITransformedGraph
        ...
        public GraphNode PointOnNavmesh (Int3 position, NNConstraint constraint) {
			...
			if (tile != null)
			{
                ...
                GraphNode node = tile.bbTree.QueryInside((Vector3)position, constraint);
				return node;
			}
        ...
        public static bool Linecast (NavmeshBase graph, Int3 origin, Int3 end, GraphNode hint, out GraphHitInfo hit, List<GraphNode> trace) {
            ...
            hit.origin = graph.transform.Transform(i3originInGraphSpace);
			...
			while (true) {
				...
				if (shapeEdgeA == 0xFF) {
					...
					if (!success) {
                        ...
                        var hitEdgeStartInGraphSpace = (shapeEdgeA == 0 ? a0 : (shapeEdgeA == 1 ? a1 : a2));
						var hitEdgeEndInGraphSpace = (shapeEdgeA == 0 ? a1 : (shapeEdgeA == 1 ? a2 : a0));
					    //Good Game
                        ...
                        var intersectionInGraphSpace = IntMath.LineIntersectionPointXZ(hitEdgeStartInGraphSpace, hitEdgeEndInGraphSpace, i3originInGraphSpace, i3endInGraphSpace);
