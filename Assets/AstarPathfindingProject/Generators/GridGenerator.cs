	...
	public class GridGraph : NavGraph, IUpdatableGraph, ITransformedGraph
        ...
        public override NNInfoInternal GetNearestForce (Int3 position, NNConstraint constraint) {
			...
			if (constraint == null || constraint.Suitable(node)) {
                ...
                minDist = (minNode.position-globalPosition).sqrMagnitude;
			    //Good Game
			...
			for (int w = 1;; w++) {
				...
				for (nx = x-w; nx <= x+w; nx++) {
					...
					if (constraint == null || constraint.Suitable(nodes[nx+nz2]))
					{
                        ...
                        float dist = (nodes[nx+nz2].position-globalPosition).sqrMagnitude;
				...
				for (nz = z-w+1; nz <= z+w-1; nz++) {
					...
					if (constraint == null || constraint.Suitable(nodes[nx+nz*width]))
					{
                        ...
                        float dist = (nodes[nx+nz*width].position-globalPosition).sqrMagnitude;
						if (dist < minDist && dist < maxDistSqr) {
							minDist = dist;
        ...
        protected bool ClipLineSegmentToBounds (Int3 a, Int3 b, out Int3 outA, out Int3 outB) {
			...
			if (a.x < 0 || a.z < 0 || a.x > width || a.z > depth ||
                ...
                intersection = IntMath.SegmentIntersectionPointXZ(a, b, p1, p2, out intersect);
                ...
                intersection = IntMath.SegmentIntersectionPointXZ(a, b, p2, p3, out intersect);
                ...
                intersection = IntMath.SegmentIntersectionPointXZ(a, b, p3, p4, out intersect);
                ...
                intersection = IntMath.SegmentIntersectionPointXZ(a, b, p4, p1, out intersect);
        ...
        public bool Linecast (Int3 from, Int3 to, GraphNode hint, out GraphHitInfo hit, List<GraphNode> trace) {
            ...
            // Find the closest nodes to the start and end on the part of the segment which is on the graphLinecast
		    //Good Game
			...
			while (current.NodeInGridIndex != endNode.NodeInGridIndex) {
				...
				if (other != null) {
                    ...
                    var intersection = IntMath.LineIntersectionPoint(lineOrigin, lineOrigin+lineDirection, start, end);

					//var currentNodePositionInGraphSpace = transform.InverseTransform((Vector3)current.position);
					var currentNodePositionInGraphSpace = transform.InverseTransform(current.position);
        ...
        public bool SnappedLinecast (Int3 from, Int3 to, GraphNode hint, out GraphHitInfo hit) {
			return Linecast(
                ...
                GetNearest(from, NNConstraint.None).node.position,
				GetNearest(to, NNConstraint.None).node.position,
				hint,
				out hit
				);
		}
