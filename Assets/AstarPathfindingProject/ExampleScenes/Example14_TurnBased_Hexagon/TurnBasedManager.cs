	...
	public class TurnBasedManager : MonoBehaviour {
		...
		static IEnumerator MoveAlongPath (TurnBasedAI unit, ABPath path, float speed) {
			...
			for (int i = 0; i < path.vectorPath.Count - 1; i++) {
                ...
                var segmentLength = Vector3.Distance((Vector3)p1, (Vector3)p2);

				while (distanceAlongSegment < segmentLength)
				{
				    //Good Game
                    //var interpolatedPoint = AstarSplines.CatmullRom(p0, p1, p2, p3, distanceAlongSegment / segmentLength);
                    var interpolatedPoint = AstarSplines.CatmullRom((Vector3)p0, (Vector3)p1, (Vector3)p2, (Vector3)p3, distanceAlongSegment / segmentLength);
            ...
            unit.transform.position = (Vector3)path.vectorPath[path.vectorPath.Count - 1];
		}
