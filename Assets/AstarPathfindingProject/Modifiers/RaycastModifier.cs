...
namespace Pathfinding {
	...
	public class RaycastModifier : MonoModifier {
		...
		public override void Apply (Path p) {
		    ...
		    //Good Game
            if (ValidateLine(null, null, (Vector3)p.vectorPath[0], (Vector3)p.vectorPath[p.vectorPath.Count-1])) {
        ...
        List<Int3> ApplyGreedy (Path p, List<Int3> points) {
			...
			while (startIndex < points.Count)
			{
				...
				while (true) {
                    ...
                    if (!ValidateLine(startNode, endNode, (Vector3)start, (Vector3)end)) break;
					mn = mx;
					mx *= 2;
				}
				...
				while (mn + 1 < mx) {
                    ...
                    if (ValidateLine(startNode, endNode, (Vector3)start, (Vector3)end)) {
						mn = mid;
					} else {
						mx = mid;
					}
			...
			for (int i = 0; i < points.Count; i++) {
				...
				for (int j = i+1; j < points.Count; j++) {
					...
					if (DPParents[j] == -1 || d2 < DPCosts[j])
					{
                        ...
                        var endIsOriginalNode = canBeOriginalNodes && points[j] == p.path[j].position;
					    //Good Game
                        //if (j == i+1 || ValidateLine(startIsOriginalNode ? p.path[i] : null, endIsOriginalNode ? p.path[j] : null, start, points[j])) {
                        if (j == i+1 || ValidateLine(startIsOriginalNode ? p.path[i] : null, endIsOriginalNode ? p.path[j] : null, (Vector3)start, (Vector3)points[j])) {
							DPCosts[j] = d2;
							DPParents[j] = i;
						} else {
							break;
