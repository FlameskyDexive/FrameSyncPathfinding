...
namespace Pathfinding {
	...
	public class GTileHandlerHelper : VersionedMonoBehaviour {
		TileHandler handler;
		...
		public void DiscardPending () {
			if (handler != null) {
				for (var cut = handler.gcuts.AllItems; cut != null; cut = cut.next) {
		...
		public void ForceUpdate () {
			...
			var allCuts = handler.gcuts.AllItems;
			...
			for (var cut = allCuts; cut != null; cut = cut.next) {
				if (cut.obj.RequiresUpdate()) {
					...
					handler.gcuts.Move(cut.obj, newTouchingTiles);
